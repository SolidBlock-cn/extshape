...
public final class ExtShapeBlocks {
  ...
  static {
    ...
    for (final Block block : new Block[]{PACKED_MUD, MUD_BRICKS}) {
      BlocksBuilder.createComprehensive(block, Items.MUD, ButtonType.STONE, ActivationRule.MOBS).build();
    }
    ...
    for (final Block block : ExtShapeBlockTags.GLAZED_TERRACOTTA) {
      new SlabBuilder(block).setInstanceSupplier(builder -> new GlazedTerracottaSlabBlock(builder.baseBlock, Block.Settings.copy(builder.baseBlock))).setDefaultTagToAdd(ExtShapeBlockTags.GLAZED_TERRACOTTA_SLABS).build();
    }
