...
public final class ExtShapeBlocks {
   ...
   */
  public static final ObjectSet<Block> BLOCKS = new ObjectLinkedOpenHashSet<>();
  /**
   * 石化橡木方块。
   */
  ...
  static {
    ...
    for (final Block block : ExtShapeTags.STONES) {
      BlocksBuilder.createAllShapes(block, FenceSettings.common(Items.FLINT), ButtonType.STONE, ActivationRule.MOBS).build();
    }

    // 泥土和砂土。其中砂土没有按钮和压力板。
    BlocksBuilder.createAllShapes(DIRT, FenceSettings.common(Items.STICK), ButtonType.SOFT, ActivationRule.EVERYTHING).build();
    BlocksBuilder.createAllShapes(COARSE_DIRT, FenceSettings.common(Items.STICK), null, null).build();
    ...
    // 基岩。
    BlocksBuilder.createAllShapes(BEDROCK, FenceSettings.common(Items.STICK), ButtonType.HARD, ActivationRule.MOBS)
        .consumeEachSettings((shape, settings) -> settings.strength(-1.0F, 3600000.0F).allowsSpawning((state, world, pos, type) -> false))
        .build();

    // 青金石块。
    BlocksBuilder.createAllShapes(LAPIS_BLOCK, FenceSettings.common(Items.LAPIS_LAZULI), ButtonType.STONE, ActivationRule.MOBS).build();
    ...
    for (final Block block : new Block[]{SMOOTH_SANDSTONE, SMOOTH_RED_SANDSTONE}) {
      BlocksBuilder.createEmpty(block).withFences(FenceSettings.common(Items.STICK)).withPressurePlate(ActivationRule.MOBS).withButton(ButtonType.STONE).build();
    }

    // 羊毛。
    for (final Block block : ExtShapeTags.WOOLS) {
      BlocksBuilder.createAllShapes(block, FenceSettings.common(Items.STRING), ButtonType.SOFT, ActivationRule.EVERYTHING)
    ...
    // 金块。
    BlocksBuilder.createAllShapes(GOLD_BLOCK, FenceSettings.common(Items.GOLD_INGOT), ButtonType.STONE, null).build();
    // 铁块。
    BlocksBuilder.createAllShapes(IRON_BLOCK, FenceSettings.common(Items.IRON_INGOT), ButtonType.STONE, null).build();

    // 砖栅栏和栅栏门。
    BlocksBuilder.createConstructionOnly(BRICKS).withFences(FenceSettings.common(Items.BRICK)).with(BlockShape.WALL).build();

    // 苔石栅栏和栅栏门。
    BlocksBuilder.createAllShapes(MOSSY_COBBLESTONE, FenceSettings.common(Items.STICK), ButtonType.STONE, ActivationRule.MOBS).build();

    // 黑曜石。
    BlocksBuilder.createAllShapes(OBSIDIAN, FenceSettings.common(Items.STONE), ButtonType.HARD, ActivationRule.MOBS).build();

    // 钻石块。
    BlocksBuilder.createAllShapes(DIAMOND_BLOCK, FenceSettings.common(Items.DIAMOND), ButtonType.HARD, ActivationRule.MOBS).build();

    // 紫水晶块。
    BlocksBuilder.createAllShapes(AMETHYST_BLOCK, FenceSettings.common(Items.AMETHYST_SHARD), ButtonType.STONE, ActivationRule.MOBS).withoutRedstone().build();

    // 冰，由于技术原因，暂不产生。

    // 雪块。
    BlocksBuilder.createAllShapes(SNOW_BLOCK, FenceSettings.common(Items.SNOW), ButtonType.SOFT, ActivationRule.EVERYTHING).build();

    // 黏土块。
    BlocksBuilder.createAllShapes(CLAY, FenceSettings.common(Items.CLAY_BALL), ButtonType.SOFT, ActivationRule.EVERYTHING).build();

    // 南瓜。
    BlocksBuilder.createAllShapes(PUMPKIN, FenceSettings.common(Items.PUMPKIN_SEEDS), ButtonType.WOODEN, ActivationRule.EVERYTHING).build();

    // 下界岩。
    BlocksBuilder.createAllShapes(NETHERRACK, FenceSettings.common(Items.NETHER_BRICK), ButtonType.STONE, ActivationRule.MOBS).build();

    // 荧石可以发光。
    BlocksBuilder.createAllShapes(GLOWSTONE, FenceSettings.common(Items.GLOWSTONE_DUST), ButtonType.SOFT, ActivationRule.EVERYTHING).build();

    // 石砖、苔石砖。
    for (final Block block : new Block[]{STONE_BRICKS, MOSSY_STONE_BRICKS, CHISELED_STONE_BRICKS}) {
      BlocksBuilder.createConstructionOnly(block).withFences(FenceSettings.common(Items.FLINT)).with(BlockShape.WALL).build();
    }

    // 泥（自 1.19）。
    for (final Block block : new Block[]{PACKED_MUD, MUD_BRICKS}) {
      BlocksBuilder.createAllShapes(block, FenceSettings.common(Items.MUD), ButtonType.STONE, ActivationRule.MOBS).build();
    }

    // 西瓜。
    BlocksBuilder.createAllShapes(MELON, FenceSettings.common(Items.MELON_SLICE), ButtonType.WOODEN, ActivationRule.EVERYTHING).build();

    // 下界砖块的栅栏门、按钮和压力板。
    BlocksBuilder.createAllShapes(NETHER_BRICKS, FenceSettings.common(Items.NETHER_BRICK), ButtonType.STONE, ActivationRule.MOBS).withoutRedstone().build();

    // 末地石、末地石砖。
    BlocksBuilder.createAllShapes(END_STONE, FenceSettings.common(Items.END_STONE_BRICKS), ButtonType.STONE, ActivationRule.MOBS).build();

    // 绿宝石块。
    BlocksBuilder.createAllShapes(EMERALD_BLOCK, FenceSettings.common(Items.EMERALD), ButtonType.STONE, ActivationRule.MOBS).build();

    // 石英、石英砖、平滑石英块、錾制石英块均有按钮和压力板。
    for (final Block block : new Block[]{QUARTZ_BLOCK, CHISELED_QUARTZ_BLOCK, QUARTZ_BRICKS, SMOOTH_QUARTZ}) {
      BlocksBuilder.createAllShapes(block, FenceSettings.common(Items.QUARTZ), ButtonType.STONE, ActivationRule.MOBS).build();
    }

    // 红色下界砖。
    BlocksBuilder.createAllShapes(RED_NETHER_BRICKS, FenceSettings.common(Items.NETHER_BRICK), ButtonType.STONE, ActivationRule.MOBS).build();

    // 陶瓦和彩色陶瓦。
    BlocksBuilder.createAllShapes(TERRACOTTA, FenceSettings.common(Items.CLAY), ButtonType.STONE, ActivationRule.MOBS)
    ...
    for (final Block block : ExtShapeTags.STAINED_TERRACOTTA) {
      BlocksBuilder.createAllShapes(block, FenceSettings.common(Items.CLAY), ButtonType.STONE, ActivationRule.MOBS)
    ...
    // 煤炭块。
    BlocksBuilder.createAllShapes(COAL_BLOCK, FenceSettings.common(Items.COAL), ButtonType.STONE, ActivationRule.MOBS).build();

    // 浮冰。
    BlocksBuilder.createAllShapes(PACKED_ICE, FenceSettings.common(Items.ICE), ButtonType.STONE, ActivationRule.MOBS).build();

    // 海晶石、海晶石砖、暗海晶石。
    for (final Block block : new Block[]{PRISMARINE, PRISMARINE_BRICKS, DARK_PRISMARINE}) {
      BlocksBuilder.createAllShapes(block, FenceSettings.common(Items.PRISMARINE_SHARD), ButtonType.STONE, ActivationRule.MOBS).build();
    }
    ...
    BlocksBuilder.createAllShapes(SMOOTH_STONE, FenceSettings.common(Items.FLINT), ButtonType.STONE, ActivationRule.MOBS).withoutConstructionShapes().build();

    BlockMappings.getMappingOf(BlockShape.SLAB).put(SMOOTH_STONE_DOUBLE_SLAB, SMOOTH_STONE_SLAB);

    // 紫珀块。
    BlocksBuilder.createAllShapes(PURPUR_BLOCK, FenceSettings.common(Items.SHULKER_SHELL), ButtonType.STONE, ActivationRule.MOBS).build();

    // 下界疣块、诡异疣块。
    BlocksBuilder.createAllShapes(NETHER_WART_BLOCK, FenceSettings.common(Items.NETHER_WART), null, null).withoutRedstone().build();
    BlocksBuilder.createAllShapes(WARPED_WART_BLOCK, FenceSettings.common(Items.NETHER_WART), null, null).withoutRedstone().build();
    ...
    for (final Block block : ExtShapeTags.CONCRETES) {
      BlocksBuilder.createAllShapes(block, FenceSettings.common(Items.GRAVEL), ButtonType.STONE, ActivationRule.MOBS)
    ...
    // 菌光体。
    BlocksBuilder.createAllShapes(SHROOMLIGHT, FenceSettings.common(Items.GLOWSTONE_DUST), ButtonType.SOFT, ActivationRule.EVERYTHING).build();

    // 蜜脾块。
    BlocksBuilder.createAllShapes(HONEYCOMB_BLOCK, FenceSettings.common(Items.HONEYCOMB), ButtonType.SOFT, ActivationRule.EVERYTHING).build();

    // 下界合金方块。
    BlocksBuilder.createAllShapes(NETHERITE_BLOCK, FenceSettings.common(Items.NETHERITE_INGOT), ButtonType.HARD, ActivationRule.MOBS).build();

    // 远古残骸。
    BlocksBuilder.createAllShapes(ANCIENT_DEBRIS, FenceSettings.common(Items.NETHERITE_SCRAP), ButtonType.HARD, ActivationRule.MOBS).build();

    // 哭泣的黑曜石。
    BlocksBuilder.createAllShapes(CRYING_OBSIDIAN, FenceSettings.common(Items.STONE), ButtonType.HARD, ActivationRule.MOBS).build();
    ...
    // 凝灰岩，方解石。
    BlocksBuilder.createAllShapes(TUFF, FenceSettings.common(Items.FLINT), ButtonType.STONE, ActivationRule.MOBS).build();
    BlocksBuilder.createAllShapes(CALCITE, FenceSettings.common(Items.FLINT), ButtonType.STONE, ActivationRule.MOBS).build();
    ...
    for (final Block block : new Block[]{
      ...
      BlocksBuilder.createAllShapes(block, FenceSettings.common(Items.COPPER_INGOT), ButtonType.STONE, ActivationRule.MOBS).build();
    }

    // 滴水石、苔藓。
    BlocksBuilder.createAllShapes(DRIPSTONE_BLOCK, FenceSettings.common(Items.POINTED_DRIPSTONE), ButtonType.STONE, ActivationRule.MOBS).build();
    BlocksBuilder.createAllShapes(MOSS_BLOCK, FenceSettings.common(Items.MOSS_CARPET), ButtonType.SOFT, ActivationRule.EVERYTHING).build();

    // 深板岩。
    BlocksBuilder.createAllShapes(DEEPSLATE, FenceSettings.common(Items.FLINT), ButtonType.STONE, ActivationRule.MOBS).setPillar().build();
    for (final Block block : new Block[]{COBBLED_DEEPSLATE, POLISHED_DEEPSLATE, DEEPSLATE_TILES, DEEPSLATE_BRICKS, CHISELED_DEEPSLATE}) {
      BlocksBuilder.createAllShapes(block, FenceSettings.common(Items.FLINT), ButtonType.STONE, ActivationRule.MOBS).build();
    }

    // 玄武岩及其变种。
    BlocksBuilder.createAllShapes(BASALT, FenceSettings.common(Items.FLINT), ButtonType.STONE, ActivationRule.MOBS).setPillar().build();
    BlocksBuilder.createAllShapes(POLISHED_BASALT, FenceSettings.common(Items.FLINT), ButtonType.STONE, ActivationRule.MOBS).setPillar().build();
    BlocksBuilder.createAllShapes(SMOOTH_BASALT, FenceSettings.common(Items.FLINT), ButtonType.STONE, ActivationRule.MOBS).build();

    // 粗铁、粗铜、粗金。
    BlocksBuilder.createAllShapes(RAW_IRON_BLOCK, FenceSettings.common(Items.RAW_IRON), ButtonType.STONE, ActivationRule.MOBS).build();
    BlocksBuilder.createAllShapes(RAW_COPPER_BLOCK, FenceSettings.common(Items.RAW_COPPER), ButtonType.STONE, ActivationRule.MOBS).build();
    BlocksBuilder.createAllShapes(RAW_GOLD_BLOCK, FenceSettings.common(Items.RAW_GOLD), ButtonType.STONE, ActivationRule.MOBS).build();

    // 蛙明灯。
    BlocksBuilder.createAllShapes(OCHRE_FROGLIGHT, FenceSettings.common(Items.SLIME_BALL), ButtonType.WOODEN, ActivationRule.EVERYTHING).setPillar().build();
    BlocksBuilder.createAllShapes(VERDANT_FROGLIGHT, FenceSettings.common(Items.SLIME_BALL), ButtonType.WOODEN, ActivationRule.EVERYTHING).setPillar().build();
    BlocksBuilder.createAllShapes(PEARLESCENT_FROGLIGHT, FenceSettings.common(Items.SLIME_BALL), ButtonType.WOODEN, ActivationRule.EVERYTHING).setPillar().build();
