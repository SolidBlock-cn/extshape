...
public final class VanillaStonecutting {
  ...
  static {
    ...
    if (FabricLoader.getInstance().isDevelopmentEnvironment()) {
      ...
      if (!unrecognizedBaseBlocks.isEmpty()) {
        throw new IllegalStateException("These blocks exist in the results of VanillaStonecutting.INSTANCE, but are not base blocks: " + unrecognizedBaseBlocks.stream().map(BuiltInRegistries.BLOCK::getKey).map(Objects::toString).collect(Collectors.joining(", ")));
      }
    }
  ...
  private static void registerVanillaStonecutting(ImmutableMultimap.Builder<Block, Block> builder) {
    ...
    // 26.2 加入的硫黄和朱砂
    builder.put(Blocks.SULFUR_BRICKS, Blocks.SULFUR);
    builder.put(Blocks.POLISHED_SULFUR, Blocks.SULFUR);
    builder.put(Blocks.SULFUR_BRICKS, Blocks.POLISHED_SULFUR);
    builder.put(Blocks.CINNABAR_BRICKS, Blocks.CINNABAR);
    builder.put(Blocks.POLISHED_CINNABAR, Blocks.CINNABAR);
    builder.put(Blocks.CINNABAR_BRICKS, Blocks.POLISHED_CINNABAR);
  }
