...
public abstract class AbstractBlockBuilder<T extends Block> {
   ...
   */
  public @Nullable BlockItem itemInstance;
  /**
   ...
   */
  protected @Nullable BlockShape shape;
  /**
   ...
   * 用于构造其物品实例的匿名函数。
   */
  protected Function<AbstractBlockBuilder<T>, BlockItem> itemInstanceSupplier = builder -> new ExtShapeBlockItem(Objects.requireNonNull(builder.instance, "instance"), builder.itemSettings);

  protected AbstractBlockBuilder(@Nullable Block baseBlock, BlockBehaviour.Properties settings, Function<AbstractBlockBuilder<T>, T> instanceSupplier) {
    ...
    if (baseBlock != null && baseBlock.asItem() != Items.AIR) {
      ...
      if (component != null) {
        if (DamageTypeTags.IS_FIRE.equals(component.types())) {
  ...
  protected AbstractBlockBuilder(Block baseBlock, Function<AbstractBlockBuilder<T>, T> instanceSupplier) {
   ...
   * @return 转换得到的路径前缀。
   */
  public static String getPathPrefixOf(String path) {
   ...
   */
  public static Identifier convertIdentifier(Identifier identifier, @Nullable String namespace, String suffix) {
  ...
  public Identifier getBlockId() {
    if (identifier == null) {
      identifier = convertIdentifier(BuiltInRegistries.BLOCK.getKey(Preconditions.checkNotNull(baseBlock, "basedBlock must not be null to create a block ID")), defaultNamespace, Preconditions.checkNotNull(this.getSuffix(), "suffix must not be null to create a block ID"));
    }
    return identifier;
  }
  ...
  public T build() {
    ...
    if (buildItem) {
      createItemInstance();
      if (registerItem) {
        Registry.register(BuiltInRegistries.ITEM, blockId, this.itemInstance);
      }
    }
