...
public abstract class AbstractBlockBuilder<T extends Block> {
  ...
  protected AbstractBlockBuilder(@Nullable Block baseBlock, AbstractBlock.Settings settings, Function<AbstractBlockBuilder<T>, T> instanceSupplier) {
  ...
  public Identifier getBlockId() {
    if (identifier == null) {
      identifier = convertIdentifier(Registries.BLOCK.getId(Preconditions.checkNotNull(baseBlock, "basedBlock must not be null to create a block ID")), defaultNamespace, Preconditions.checkNotNull(this.getSuffix(), "suffix must not be null to create a block ID"));
    }
    return identifier;
  }
