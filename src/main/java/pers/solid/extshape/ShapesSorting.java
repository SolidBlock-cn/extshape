...
public class ShapesSorting implements Supplier<Collection<Map<Item, Collection<Item>>>> {
    ...
    public static final Map<Block, Collection<Block>> ABSTRACT_BLOCK_SHAPE_SORTING_RULES = new AbstractMap<Block, Collection<Block>>() {
        @NotNull
        @Override
        public Set<Entry<Block, Collection<Block>>> entrySet() {
            return BlockMappings.BASE_BLOCKS.stream().map(block -> new SimpleImmutableEntry<>(block, get(block))).filter(entry -> entry.getValue() != null).collect(Collectors.toSet());
        }
        ...
        public Collection<Block> get(Object key) {
            ...
            final List<Block> blocks = SHAPES.stream().map(shape -> BlockMappings.getBlockOf(shape, ((Block) key))).filter(Objects::nonNull).collect(Collectors.toList());
            return blocks.isEmpty() ? null : blocks;
        }
    };
    public static final Map<Item, Collection<Item>> ABSTRACT_ITEM_SHAPE_SORTING_RULES = new AbstractMap<Item, Collection<Item>>() {
        @NotNull
        @Override
        public Set<Entry<Item, Collection<Item>>> entrySet() {
            return ABSTRACT_BLOCK_SHAPE_SORTING_RULES.entrySet().stream().map(entry -> new SimpleImmutableEntry<Item, Collection<Item>>(entry.getKey().asItem(), entry.getValue().stream().map(Block::asItem).collect(Collectors.toList()))).filter(entry -> entry.getKey() != null && entry.getValue() != null).collect(Collectors.toSet());
        }
        ...
        public Collection<Item> get(Object key) {
            ...
            return blocks == null ? null : blocks.stream().map(Block::asItem).filter(Objects::nonNull).collect(Collectors.toList());
        }
    };
