...
public final class UnusualLootTables {
   ...
   * 只有带有精准采集附魔时才会掉落的方块。
   */
  public static final LootTableFunction DROPS_DOUBLE_WITH_SILK_TOUCH_OR_NONE = (baseBlock, shape, block) -> dropsDoubleSlabWithSilkTouchOrNone(block, shape == BlockShape.SLAB);
  ...
  public static LootTableFunction dropsWithSilkTouchOrConst(@NotNull ItemConvertible drop, float fullCount) {
    return (baseBlock, shape, block) -> {
      ...
      if (shape == BlockShape.SLAB) {
        return LootTable.builder().pool(LootPool.builder().with(ItemEntry.builder(drop).apply(SetCountLootFunction.builder(ConstantLootNumberProvider.create(2)).conditionally(BlockStatePropertyLootCondition.builder(block).properties(EXACT_MATCH_DOUBLE_SLAB))).conditionally(WITH_SILK_TOUCH).alternatively(entryBuilder)));
      } else {
