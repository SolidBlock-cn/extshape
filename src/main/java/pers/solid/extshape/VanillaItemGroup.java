...
public final class VanillaItemGroup {
  ...
  public static void registerForMod() {
    final Multimap<Item, Item> apRedstone = getAppendingRule(Registries.ITEM_GROUP.getOrThrow(ItemGroups.REDSTONE));
    apRedstone.put(Items.STONE_BUTTON, Objects.requireNonNull(BlockBiMaps.getBlockOf(BlockShape.BUTTON, Blocks.OBSIDIAN)).asItem());
    final Multimap<Item, Item> preRedstone = getPrependingRule(Registries.ITEM_GROUP.getOrThrow(ItemGroups.REDSTONE));
  ...
  public static void recreateVanillaGroupRules(Collection<BlockShape> shapes) {
    final Multimap<Item, Item> apBuilding = getAppendingRule(Registries.ITEM_GROUP.getOrThrow(ItemGroups.BUILDING_BLOCKS));
    final Multimap<Item, Item> preBuilding = getPrependingRule(Registries.ITEM_GROUP.getOrThrow(ItemGroups.BUILDING_BLOCKS));
    ...
    new EntryVariantAppender(Registries.ITEM_GROUP.getOrThrow(ItemGroups.BUILDING_BLOCKS), shapes, Iterables.filter(BlockBiMaps.BASE_BLOCKS, block -> !(BlockCollections.WOOLS.contains(block) || BlockCollections.STAINED_TERRACOTTA.contains(block) || BlockCollections.CONCRETES.contains(block) || BlockCollections.GLAZED_TERRACOTTA.contains(block) || block == Blocks.TERRACOTTA)), ExtShapeBlocks.getBlocks()::contains).appendItems(apBuilding);
    new EntryVariantAppender(Registries.ITEM_GROUP.getOrThrow(ItemGroups.COLORED_BLOCKS), shapes, Iterables.concat(BlockCollections.WOOLS, Collections.singleton(Blocks.TERRACOTTA), BlockCollections.STAINED_TERRACOTTA, BlockCollections.CONCRETES, BlockCollections.GLAZED_TERRACOTTA), ExtShapeBlocks.getBlocks()::contains).appendItems(getAppendingRule(Registries.ITEM_GROUP.getOrThrow(ItemGroups.COLORED_BLOCKS)));
    new EntryVariantAppender(Registries.ITEM_GROUP.getOrThrow(ItemGroups.NATURAL), shapes, Iterables.filter(BlockBiMaps.BASE_BLOCKS, block -> !(BlockCollections.LOGS.contains(block) || BlockCollections.STEMS.contains(block))), ExtShapeBlocks.getBlocks()::contains).appendItems(getAppendingRule(Registries.ITEM_GROUP.getOrThrow(ItemGroups.NATURAL)));
  }
}
