...
This document is also [provided in English](README-en.md).

本模组为许多的原版方块添加了其楼梯、台阶、栅栏、栅栏门、按钮和压力板，以及原版不具有的竖直台阶、竖直楼梯、横条、纵条。具体内容列表请参考[方块列表](BlockList.md)。

模组目前支持 Minecraft Java 版 1.21.1-1.16.5，请根据你的游戏版本安装对应的模组，并需要 Fabric API才能运行。此外，对于 Fabric，通常还需要 Mod Menu（非必须）以打开模组配置界面。
...
一些配方可能与原版冲突，为了避免冲突，本模组内置了防冲突配方的数据包，默认开启。启用该数据包后，以下合成有变化：

- 羊毛的压力板不是由两个羊毛合成，而是由 3 个地毯合成 1 个羊毛压力板，1 个羊毛压力板也可以合成为 3 个地毯。
- 苔藓压力板不是由两个苔藓方块合成，而是由 1 个覆地苔藓合成 1 个苔藓压力板，1 个苔藓压力板也可以合成为 2 个覆地苔藓。
...
- 竹块、去皮竹块、铜块（含各级锈蚀变种，不含切制铜）、涂蜡的铜块（含各级锈蚀变种，不含涂蜡的切制铜），各种原木、木头、菌柄、菌核及其去皮变种，由 1 个基础方块和 1 个红石粉合成。
...
你还可以在模组配置界面设置添加哪些形状的方块。可以通过 Mod Menu 的模组菜单进入模组的配置页面，如果没有安装 Mod Menu 模组，也可以在游戏内输入 `extshape:config` 进入模组的配置界面。
