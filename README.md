...
> 自 3.0.0 版本开始，不再需要安装[更好的运行时资源包（BRRP）](https://github.com/SolidBlock-cn/BRRP/)模组。
...
另外此模组还添加了“双层平滑石台阶”方块和“石化橡木木板”方块，分别可以合成平滑石台阶和石化橡木台阶（原版的平滑石块也可以合成平滑石台阶）。这两个方块均可装进硫方怪，其硫方怪原型均为 `#slow_bouncy`（和石头一样）。
...
该数据包是默认启用的，如果您的世界没有启用，可运行命令 `/datapack enable "extshape:recipe_tweak"`。

> 如果你安装了与其他模组联动的部分（见[#模组联动](#模组联动)），那么这些配方的冲突是无法避免的，这种情况下建议直接安装能修复合成表冲突的模组。
