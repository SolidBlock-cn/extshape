...
public final class ExtShapeBlockusBlocks {
  ...
  static {
    ...
    create(BlockusBlocks.BAMBOO).setPrimaryTagForShape(ExtShapeTags.SHAPE_TO_WOODEN_TAG).setActivationSettings(ActivationSettings.BAMBOO).setFenceSettings(new FenceSettings(Items.BAMBOO, WoodType.BAMBOO)).addPostBuildConsumer(plankFlammable).addPostBuildConsumer(plankFuel).build();
    ...
    for (var patternWoolTypes : BlockusBlockCollections.PATTERNED_WOOLS) {
      FACTORY.createAllShapes(patternWoolTypes.block)
          ...
          .addPreBuildConsumer((blockShape, blockBuilder) -> {
            if (blockShape == BlockShape.PRESSURE_PLATE) {
              ((PressurePlateBuilder) blockBuilder).setInstanceSupplier(x -> new WoolPressurePlate(x.baseBlock, x.blockSettings, ((PressurePlateBuilder) x).activationSettings, patternWoolTypes.carpet));
            }
          })
    ...
    create(BlockusBlocks.ROTTEN_FLESH_BLOCK).addExtraTag(BlockTags.HOE_MINEABLE).addExtraTag(addWallToUnmineableTag).setActivationSettings(ActivationSettings.soft(ExtShapeBlockusBlockTypes.SLIME_BLOCK_SET_TYPE)).setFenceSettings(new FenceSettings(Items.ROTTEN_FLESH, ExtShapeBlockusBlockTypes.SLIME_WOOD_TYPE)).build();
    create(BlockusBlocks.MEMBRANE_BLOCK).setFenceSettings(new FenceSettings(Items.PHANTOM_MEMBRANE, ExtShapeBlockusBlockTypes.SLIME_WOOD_TYPE)).setActivationSettings(ActivationSettings.soft(ExtShapeBlockusBlockTypes.SLIME_BLOCK_SET_TYPE)).addExtraTag(addWallToUnmineableTag).build();
  ...
  private static BlocksBuilder create(WoodTypesF woodTypes) {
  ...
  private static BlocksBuilder create(WoodTypesFP woodTypes) {
    return create(woodTypes.planks).without(BlockShape.STAIRS, BlockShape.SLAB, BlockShape.FENCE, BlockShape.FENCE_GATE, BlockShape.PRESSURE_PLATE, BlockShape.BUTTON);
  }
