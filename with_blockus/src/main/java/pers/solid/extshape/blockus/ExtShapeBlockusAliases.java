...
public class ExtShapeBlockusAliases {
  ...
  private static void convertWallsToBlockus(String baseName) {
    ...
    if (Registries.BLOCK.containsId(from)) {
    ...
    if (!Registries.BLOCK.containsId(to)) {
    ...
    if (Registries.ITEM.containsId(from)) {
    ...
    if (!Registries.ITEM.containsId(to)) {
    ...
    if (blockValid) {
      Registries.BLOCK.addAlias(from, to);
    }
    if (itemValid) {
      Registries.ITEM.addAlias(from, to);
    }
  }
}
