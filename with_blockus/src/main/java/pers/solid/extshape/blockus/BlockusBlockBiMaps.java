...
public final class BlockusBlockBiMaps {
  ...
  static void importFromBlockus() {
    ...
    WoolBundle.values().stream().flatMap(woolBundle -> woolBundle.colorMap().values().stream()).forEach(woolVariants -> {
    ...
    ConcreteBundle.values().stream().flatMap(concreteBundle -> concreteBundle.colorMap().values().stream()).forEach(concreteVariants -> {
